...
def gadm_to_plain(label):
        # Handles patterns like 'ID.10_1_AC', 'VN.9_1_AC battery'
        parts = label.split("_AC")
        main = parts[0]
        suffix = label[len(main) + 3:]  # include spacing and battery label if present

        if "." not in main or "_" not in main:
            return label  # skip if unexpected format

        iso, rest = main.split(".")
        rest_parts = rest.split("_")
        if len(rest_parts) < 2:
            return label  # or handle differently if needed
        gadm_id = rest_parts[0]
        sub = "_".join(rest_parts[1:])
        return f"{iso}{gadm_id} {sub}{suffix}"
...
def clustering_for_n_clusters(
    ...
    if not isinstance(custom_busmap, pd.Series):
        if alternative_clustering:
            ...
            busmap = busmap.apply(gadm_to_plain)
        else:
    ...
    if alternative_clustering:
        nc = clustering.network
        if not nc.loads.empty:
            nc.loads.index = nc.loads.bus.values
            nc.loads.index.name = "Load"

            if nc.loads.index.duplicated().any():
                keep_cols = [c for c in nc.loads.columns if c != "bus"]
                nc.loads = (
                    nc.loads[keep_cols]
                    .groupby(level=0, sort=False)
                    .sum(numeric_only=True)
                )
                nc.loads["bus"] = nc.loads.index
                nc.loads["carrier"] = "electricity"
        clustering.network.buses["country"] = clustering.network.buses.index.str[:2]
